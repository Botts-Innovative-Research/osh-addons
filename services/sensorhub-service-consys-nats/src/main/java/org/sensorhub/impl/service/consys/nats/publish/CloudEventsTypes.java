 ...
 *
 * <p>
 * Duplicated verbatim from the MQTT binding so the NATS binding stays
 * self-contained (per the "duplicate into NATS module" decision). The type
 * tokens are transport-agnostic and MUST stay identical across bindings.
 * </p>
 */
public final class CloudEventsTypes
{
    ...
    /** MIME type of a structured-mode CloudEvents JSON message (CloudEvents NATS protocol binding). */
    ...
    // Observation events
    // Note: the obs ID used in topics is the phenomenon time in epoch-ms because
